...
//		}
	...
	wxopen.snsapiBase=function(appid,code,jsonp,fn){
		...
		data["appid"] = appid;
		data["code"] = code;
		wxopen.callServ("https://api.weixin.qq.com/sns/userinfo",data,callServiceBack,jsonp);
	 ...
	 * access_token
	 */
	wxopen.snsapiAccessToken =function(service,appid,secretid,code,callback){
		var data = {};
		data["appid"] = appid;
		...
		$.ajax({
			...
			success : function(data){
				alert('AccessTokensuccess:'+wxopen.jsonToString(data));
				callback.apply(this,[data]);
			},
			error:function(data){
				alert('AccessTokenerror:'+wxopen.jsonToString(data));
				callback.apply(this,[data]);
			}
		});
	};
	...
	wxopen.callService=function(service,param,callback){
		var callback=callback||function(){};
		alert('post:'+wxopen.jsonToString(param));
		$.ajax( {
			url : service,
			data:param,
			dataType:"json",
			type : "post",	
			success : function(data){
				alert('success:'+wxopen.jsonToString(data));
				data = {head:{respCode:data.code,respDesc:data.message},body:data.data};
				callback.apply(this,[data]);
			},
			error:function(e,t){
				alert('error:'+wxopen.jsonToString(e));
				data = {head:{respCode:-1,respDesc:e.message},body:{}};
				callback.apply(this,[data]);
			}
		});
	...
	wxopen.jsonToString=function(o) {    
    var arr = []; 
    var fmt = function(s) { 
